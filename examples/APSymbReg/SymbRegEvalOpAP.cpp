...
void SymbRegEvalOpAP::addConstants(uint nConstants)
{
...
void SymbRegEvalOpAP::registerParameters(StateP stateP)
{
...
bool SymbRegEvalOpAP::initialize(StateP stateP)
{
...
FitnessP SymbRegEvalOpAP::evaluate(IndividualP individual)
{
    FitnessP fitness(new FitnessMin);

    Tree::APGenotype* apg = (Tree::APGenotype*)individual->getGenotype(index).get();

    TreeP tree = apg->convertToPhenotype();

    std::vector<uint> terminal_indices;
    std::string var = "X";
    for (uint i = 0; i < inputs[0].size(); ++i) {
        terminal_indices.push_back(apg->getTerminalIndex(var));
        var[0] += 1;
    }
    ...
    for(auto& input : inputs) {
        ...
        for (uint i = 0; i < input.size(); ++i) {
            apg->setTerminalValue(terminal_indices[i], &input[i]);
        }
        tree->execute(&result);
