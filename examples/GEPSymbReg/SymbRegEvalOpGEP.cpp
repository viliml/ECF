...
void SymbRegEvalOpGEP::addConstants(uint nConstants)
{
...
void SymbRegEvalOpGEP::registerParameters(StateP stateP)
{
...
bool SymbRegEvalOpGEP::initialize(StateP stateP)
{
...
FitnessP SymbRegEvalOpGEP::evaluate(IndividualP individual)
{
    FitnessP fitness(new FitnessMin);
    GEPChromosomeP gep = std::static_pointer_cast<GEP::GEPChromosome> (individual->getGenotype(index));
    gep->assemble();
    double value = 0.0;
    std::vector<double> calculatedOutputs;
//    std::cout<<gep->toString()<<"\n";
    for(auto& input : inputs) {
        double result = 0.0;
        for (uint i = 0; i < input.size(); ++i) {
            gep->setTerminalValue(std::string(1, 'X' + i), &input[i]);
//            std::cout<<input[i]<<" ";
        }
        gep->execute(&result);
//        std::cout<<": "<<result<<"\n";
